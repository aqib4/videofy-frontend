...
function App() {
  ...
  return (
    <>
        <Routes>
          <Route path="/" element={(isAuthenticated && isOnboarded)? (<Home/>):<Navigate to={!isAuthenticated ? "/login" : "/onbaording"} /> }/>
          <Route path="/signup" element={ !isAuthenticated ? <SignUp/> : <Navigate to={"/"} /> } />
          <Route path="/login" element={!isAuthenticated ? <Login/> : <Navigate to={isOnboarded? "/":"/onbaording"}/> } />
          <Route path="/onbaording" element={(isAuthenticated && !isOnboarded) ? <Onboarding/>  : <Navigate to={!isAuthenticated ? "/login": "/"}/> } />
          <Route path="/chat" element={(isAuthenticated && isOnboarded) ? <Chat/>  : <Navigate to={!isAuthenticated? "/login":"/chat"}/>} />
